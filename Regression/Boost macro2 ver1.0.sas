...
proc iml;
    start split_iml(x,y,w,g_info,out_type, y_pred);
        ...
        do j=1 to p;
            ...
            p1_L = (cusum(yw_sort[1:(n-1)]) # (1/lpwt));
            p0_L = (cusum(yw_sort1[1:(n-1)])# (1/lpwt));			

            rpwt = wsum - lpwt; 
			rpwt = rpwt#(rpwt >= 2*CONSTANT('SMALL')) + (rpwt < 2*CONSTANT('SMALL'))*2*CONSTANT('SMALL');
         
			p1_R = ((ywsum - cusum(yw_sort[1:(n-1)]))   # (1/rpwt));
            p0_R = ((ywsum1 - cusum(yw_sort1[1:(n-1)])) # (1/rpwt));			 
			
            gini =  p1_L#(1-p1_L)#lpwt + p1_R#(1-p1_R)#rpwt;
			

			free lpwt  rpwt  yw_cusum  yw_sort1  yw_sort  yw_sum;

            g_ord = t(gini);
            g_ord[,rank(g_ord)]=(1:n-1);
            if gini[g_ord[1,1]] < gini_min then do;
                gini_min=gini[g_ord[1,1]];
                gini_var = j;
                gini_cut = (x_sort[g_ord[1,1]] + x_sort[g_ord[1,1]+1]) / 2;
                p1_RH = p1_R[g_ord[1,1]];
                p0_RH = p0_R[g_ord[1,1]];
                p1_LH = p1_L[g_ord[1,1]];
                p0_LH = p0_L[g_ord[1,1]];
                c_R = 0;
        ...
        if out_type = 1 then do;
            do i=1 to n;
                if x[i,gini_var] <= gini_cut then y_pred[i,1] = c_L;
                if x[i,gini_var] >  gini_cut then y_pred[i,1] = c_R;
            end;
        end;
        if out_type = 2 then do;
        do i=1 to n;
            if x[i,gini_var] <= gini_cut then do;
                if c_L = 0 then y_pred[i,1] = 1-p0_LH;
                if c_L = 1 then y_pred[i,1] = p1_LH;
            end;
            if x[i,gini_var] >  gini_cut then do;
                if c_R = 0 then y_pred[i,1] = 1-p0_RH;
                if c_R = 1 then y_pred[i,1] = p1_RH;
            end;
        end; 	 
    finish split_iml;
    ...
    start regsplit_iml(x,y,w,j_info,y_pred);
        ...
        do j=1 to p;
            ...
            lpwt=y_sort*0; rpwt=1-lpwt;			
			do i=1 to (n-1);
               lpwt[i]=1; rpwt[i]=0;    
			   z = y_sort - (p1_L[i]*lpwt+p1_R[i]*rpwt);
			   css[i] =t(z)*(w_sort#z);
			end;			
		
            css_ord = css;
            css_ord[,rank(css_ord)]=(1:n-1);
            if css[css_ord[1,1]] < min_css then do;
                min_css = css[css_ord[1,1]];
                cut_val = (x_sort[css_ord[1,1]] + x_sort[css_ord[1,1]+1]) / 2;
                reg_var = j;
                wt_y = w_sort#y_sort;
                ypred_L = (sum(wt_y[1:css_ord[1,1]]))/sum(w_sort[1:css_ord[1,1]]);
                ypred_R = (sum(wt_y[css_ord[1,1]+1:n]))/
                        sum(w_sort[css_ord[1,1]+1:n]);
